...
物品 408（100 宝石 gem_*、100 存储块物品、100 板、100 辉光管、电池/二极管/电极/氧化石墨烯/石墨烯/石墨烯杆/qubit）+ 方块 100（宝石块）+ 超级组装机（方块+BE+101 槽 GUI，256×320 背景图）+ 流体 4（neon/argon/krypton/liquid_xenon 四件套齐全；修复了三者未进 `NIFluids.ALL` 导致缺客户端渲染着色的隐患）。宝石表以 `NIGems.java` 为单一来源（100 条，启动断言查重），资产脚本 `gen_optical_assets.py` 直接解析该 Java 生成材质/模型/loot/lang，杜绝两处漂移。宝石贴图为原版钻石风格刻面剪影（每色独立明暗阶梯），qubit 贴图取 MI 原版 qubit 色相平移至黄。EMI 新分类「超级组装机」（workstation=机器本体）：`NIOpticalQubitRecipe` 把 10×10=100 管格 + 箭头 + qubit 画成一页可视化配方。
