...
## 9. 2026-09-01 光学程序（optical：100 宝石 + 超级组装机，403 个新配方）

### 9.1 百宝石链（`opticalGemChain`，每宝石 4 条 ×100 + 部件 3 条）

- **宝石孕育（藻类培养器 8 EU/200t）**：玻璃×1 + 本色调律培养皿×1 + 本色辉光稀有气体 50 mB → **宝石×1（50% 概率产出）**。皿按 12 色藻轮的色相映射（消色差宝石按最近轮色），气体按 6 种稀有气体辉光色最近邻：He 桃/Ne 橙红/Ar 薰衣草/Kr 冰蓝/Xe 青/Rn 淡紫（He 用 MI 的，Ne/Ar/Kr 新注册流体）。
- **压缩（压缩机 eu2/200t）**：宝石×9 → 存储块（100 个真实可放置方块，带 loot table，nichrome 块模板重着色）。
- **切割（切割机 eu2/200t + 润滑油 1 mB）**：块×1 → 板×9（质量平衡 1 宝石 = 1 板）。
- **辉光管（装配机 2G/4000s）**：板×1 + 超铀电池×1 + 晶体二极管×1 + 石墨烯电极×2 + 本色气体 100 mB + 熔融 trinium 50 mB → 辉光管×1（真实霓虹管叙事：气体决定辉光色）。
- **部件**：超铀电池 = 钠夸德板×2 + **镅/锔/镎粉**（三种此前无下游的重元素，Am/Cm 是真实 RTG 同位素燃料）×1×1×1（2G/4000s）；晶体二极管 = 硅片×2+铜板；石墨烯电极 = 碳粉×4+铜板→2。
- **终局（机器内硬编码，非 JSON）**：**超级组装机**（新方块+BE+Menu/Screen）10×10 格子集齐全部 100 种辉光管各 ≥1 → 每次消耗各 1 → **光学量子比特组件**；无能耗无随机，收集即成本；槽位仅收辉光管，shift-点击自动入格。

### 9.2 新注册内容

物品 405（100 宝石 gem_*、100 存储块物品、100 板、100 辉光管、电池/二极管/电极/qubit）+ 方块 100（宝石块）+ 超级组装机（方块+BE+101 槽 GUI，256×320 背景图）+ 流体 3（neon/argon/krypton 四件套齐全）。宝石表以 `NIGems.java` 为单一来源（100 条，启动断言查重），资产脚本 `gen_optical_assets.py` 直接解析该 Java 生成材质/模型/loot/lang，杜绝两处漂移。EMI 新分类「超级组装机」（workstation=机器本体）：`NIOpticalQubitRecipe` 把 10×10=100 管格 + 箭头 + qubit 画成一页可视化配方。
