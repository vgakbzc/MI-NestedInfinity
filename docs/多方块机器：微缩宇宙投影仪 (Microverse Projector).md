...
产量公式：`产出 = floor(总存在 tick × 2.1^(n-1) / 20)`，整颗实时推入输出仓（不再是结束时一次性输出）。
...
5. **产出（运行中，渐进）**：物质随运行**实时产出**，整颗即推送进结构内的**物品输出仓**；
   若输出仓放不下：主方块 GUI 报错并**暂停机器**（产出停止），倒计时以 **0.5 倍速**继续减少，
   直到输出仓腾出空间自动恢复。
6. **结束**：倒计时归零 → 剩余累计物质推送输出仓（装不下的部分掉落在主方块上方）；
...
- 缺电：倒计时以 10 倍速减少（不暂停、不产出），归零即正常结束。
